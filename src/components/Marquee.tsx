...
export default function Marquee() {
  return (
    <section className="py-24 bg-surface-dim overflow-hidden w-full">
      <div className="container mx-auto px-8 md:px-12 mb-16">
          ...
          {[...logos, ...logos].map((logo, index) => (
            <div key={index} className="flex items-center justify-center shrink-0 px-4">
              <Image
                ...
                width={140}
                height={70}
                className="h-14 w-auto object-contain grayscale opacity-40 hover:grayscale-0 hover:opacity-100 transition-all duration-300 pointer-events-auto"
              />
            </div>
          ))}
        </div>
